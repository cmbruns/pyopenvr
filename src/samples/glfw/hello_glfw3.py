...
class HelloApp(object):
    "HelloApp uses glfw library to create an opengl context, listen to keyboard events, and clean up"
    last_render = None
    ...
    def init_gl(self):
        ...
        self.window = glfw.create_window(self.renderer.width, self.renderer.height, self.title, None, None)
        ...
        glfw.set_framebuffer_size_callback ( self.window, self.renderer.size_callback )
    ...
    def render_scene(self):
        ...
        if self.last_render is not None:
            print ( 'render_scene @ {}'.format ( now-self.last_render ) )
...
if __name__ == "__main__":
    ...
    with HelloApp(renderer0, "glfw OpenVR color cube") as app:
        app.run_loop()